          ...
          if (photographerInfo && photographerInfo._openid) {
            try {
              ...
              if (userRes.data && userRes.data.length > 0) {
                ...
                console.log('✅ [管理员订单详情] 从 users 集合获取摄影师头像:', userData.avatarUrl);
              } else {
                console.warn('⚠️ [管理员订单详情] 未找到摄影师的 users 记录，使用 photographers 的 avatar');
                photographerInfo.avatarUrl = photographerInfo.avatar || '';
              }
            } catch (userErr) {
              console.error('❌ [管理员订单详情] 从 users 集合查询摄影师头像失败:', userErr);
              photographerInfo.avatarUrl = photographerInfo.avatar || '';
            }
          } else {
            console.warn('⚠️ [管理员订单详情] 摄影师没有 _openid，使用 photographers 的 avatar');
            photographerInfo.avatarUrl = photographerInfo.avatar || '';
          }
        } catch (e) {
